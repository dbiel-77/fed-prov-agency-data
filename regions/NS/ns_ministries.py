...
Source: https://novascotia.ca/government/
"""
import sys
import re
...
INDEX_URL = "https://novascotia.ca/government/"
BASE = "https://novascotia.ca"

# Domains to accept as NS government entities
_GOV_DOMAINS = re.compile(
    r"novascotia\.ca|gov\.ns\.ca|nsapprenticeship\.ca|nshealth\.ca|"
    r"oag-ns\.ca|electionsnovascotia\.ca|buildns\.ca|nscudic\.org|"
    r"nslegislature\.ca|artgalleryofnovascotia\.ca|acadien\.novascotia",
    re.I,
)
# Domains that are NOT executive-branch departments
_EXCLUDE_DOMAINS = re.compile(r"nslegislature\.ca|courts\.ns\.ca", re.I)
# Utility link patterns to skip
_UTIL_HREFS = re.compile(
    r"#|login|sitemap|search|accessibility|feedback|connect|social-media|"
    r"regulations|phone|webmaster|legislation|contact|hours|location",
    re.I,
)
...
def _dept_links(session):
    ...
    for a in content.find_all("a", href=True):
        ...
        if not name or len(name) < 5:
            continue
        if _UTIL_HREFS.search(href):
            continue
        if _GOV_DOMAINS.search(href) and not _EXCLUDE_DOMAINS.search(href):
...
def _scrape_dept(session, item):
    url, name = item
    row = {
        "province": "NS", "type": "Department", "name": name,
    ...
    for a in soup.find_all("a", href=True):
        if re.search(r"contact|minister", a.get_text(strip=True), re.I):
            href = a["href"]
            full = href if href.startswith("http") else urljoin(BASE, href)
            if full != url:
    ...
    addr = re.search(r'\d+\s+\w[\w\s,]+(?:Street|Ave|Avenue|Drive|Road|St\.?),?\s*Halifax', page_text, re.I)
    if addr:
...
def scrape_ministries(output_file="data/NS/ministries.csv"):
    session = make_session()
    print("[NS] Fetching department list from novascotia.ca/government/…")
    links = _dept_links(session)
    print(f"[NS] Scraping {len(links)} departments concurrently…")
    rows = parallel_scrape(session, links, _scrape_dept)
    f, writer = open_writer(output_file, MINISTRY_FIELDS)
    writer.writerows(rows)
    f.close()
    print(f"[NS] Saved {len(rows)} -> {output_file}")
